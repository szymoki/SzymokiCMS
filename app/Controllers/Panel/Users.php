 ...
 * @Last Modified time: 2020-05-06 16:46:22
...
class Users extends PanelController {
  ...
  function __construct() {
    ...
    if($this->ses["level"]!=0) return redirect()->to(base_url("panel/admin"));
  ...
  public function addu(){
    ...
    if (!$validation->run($_POST)) {
      ...
      $id=$this->user_model->insert(array(
        ...
        "password"=>sha1($encryption->key.$this->request->getPost("password")),
 ...
 public function editu(){
  ...
  if (!$validation->run($_POST)) 
  {
   ...
   // return redirect()->to(base_url().'/panel/users'); 
    ...
    $encryption         = new \Config\Encryption();
    if($this->request->getPost("id")==1) $level=0; else $level=$this->request->getPost("level");//taki usmieszek:)
