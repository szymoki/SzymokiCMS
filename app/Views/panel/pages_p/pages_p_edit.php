        ...
        <form class="form-control" method="post" action="<?=base_url()?>/panel/pages_p/editu">
         ...
         <?endif;?>
            ...
            <select name="parent_id" class="form-control">
             <?if($pages_p->parent_id ==0):?>  <option value="0" >brak</option><?endif;?>

             <?foreach($pages_p_all as $item):?>
             <?if(acl("pages_p") or acl("p_".$item->id)):?>

             <?if($item->parent_id==0 and $pages_p->id!=$item->id):?>
             <option value="<?=$item->id?>" <?=$item->id==$pages_p->parent_id ? ' selected' :"" ?>><?=$item->title?></option>

             <?foreach($pages_p_all as $item2):?>
             <?if($item2->parent_id==$item->id):?>
             <?if($pages_p->id!=$item2->id):?>
            ...
            <table class="table">
                <thead>
                    <tr>
                        ...
                        <th>Nazwa:</th>
                        <th>Typ:</th>
                      <?if(acl("pages_p")):?>  <th><a href="<?=base_url()?>/panel/pages_p/menu_add/<?=$pages_p->id?>" class="btn btn-sm btn-success">Dodaj</a><?endif;?>
                        </th>
                    </tr>
                </thead>
                <tbody>
