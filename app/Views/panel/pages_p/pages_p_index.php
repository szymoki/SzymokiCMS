			...
			<tbody>
				...
				<tr style="border-top: 2px solid grey;background-color: #dbdbdb">
					...
					<td><a href="<?=base_url()?>/panel/pages_p/edit/<?=$item->id?>" class="btn btn-sm btn-primary">Edytuj</a>
						<?if(acl("pages_p")):?><a href="<?=base_url()?>/panel/pages_p/del/<?=$item->id?>" class="btn btn-sm btn-danger" onclick="return confirm('Czy napewno usunąć?')">Usuń</a><?endif;?>

					</td>
				</tr>

				<?foreach($pages_p as $item2):?>
				...
				<?foreach($pages_p as $item3):?>
				<?if($item3->parent_id!=0 and $item2->id==$item3->parent_id):?>
